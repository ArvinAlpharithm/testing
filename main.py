...
def summarize_text(text, summary_type):
    if summary_type == "Normal Summary":
        ...
        prompt = f"Summarize the text in 2 words: {text}"
    elif summary_type == "Simple Summary":
        prompt = f"Summarize the text to a 10 year kid: {text}"
    elif summary_type == "Bullet Point Summary":
        prompt = f"Summarize the text in 100 words with unordered list: {text}"
